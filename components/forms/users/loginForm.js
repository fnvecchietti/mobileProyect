...
import { withFirebaseHOC } from '../../../configs/firebase/firebase'
...
export const LoginForm = ({props, setLoggin}) => {
    ...
    const handleSingIn = async (username, password) =>{
        setLoggin(true)
        await props.firebase.loginWithEmail(username,password)
        .then((result)=>{
    ...
    return (<KeyboardAvoidingView behavior="padding" enabled>
        ...
        <Button title="Ingresar" type="clear" onPress={()=> handleSingIn(username, password)}/>
